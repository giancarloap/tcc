...
//                            };
                        ...
                        graph_data["links"] = [];
                        //var graph_nodes = [1];
                        //alert(dump(graph_nodes));
                        //criar array depois adicionar

                        function get_position(name) {
                            for (var m = 0 ; m < graph_data["nodes"].length ; m++) {
                                if (graph_data["nodes"][m]["name"] == name) {
                                    return m;
                                }
                            }
                        }
                        ...
                        var n = 0;
                        for (var source in sourceRepetitionCount) {
                            //alert(source);
                            if (!node_in_nodes(source)) {
                                node = {"name": source, "group" : n};
                                //alert(dump(node));
                                graph_data["nodes"].push(node);
                                //alert(dump(graph_data["nodes"]));
                            }
                            for (var target in sourceRepetitionCount[source]['targets']) {
                                if (!node_in_nodes(target)) {
                                    node = {"name": source, "group" : n};
                                    //alert(dump(node));
                                    graph_data["nodes"].push(node);
                                }
                        ...
                        for (var l = 0 ; l < links.length ; l++) {
                            ...
                            links[l]['value'] = 300;
                            //return 300;
                            link = {"source":get_position(links[l]['source']['name']), "target": get_position(links[l]['target']['name']), "value": links[l]['value']};
